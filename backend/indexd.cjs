...
var TwitterCldrLoader = _interopRequireWildcard(require("twitter_cldr"));
...
app.use(_express.default.static('files'));
// make a new logger
const myLogger = new _console.Console({
  stdout: _fs.default.createWriteStream("normalStdout.txt"),
  stderr: _fs.default.createWriteStream("errStdErr.txt")
});
const clog = d => {
  myLogger.log(d);
  console.log(d);
...
app.post('/pinga', async (req, res) => {
  ...
  res.json({
    'status': att.length == 1 ? 200 : 400
  });
});
app.all('/*', function (req, res, next) {
  next();
});
const bidi = (0, _bidiJs.default)();
...
function maybeRtlize(text) {
  var text = text;
  if (isHebrew(text)) {
    var bidiText = TwitterCldr.Bidi.from_string(text, {
      direction: "RTL"
    });
...
const fixtext = text => {
  ...
  const flips = bidi.getReorderSegments(text,
  ...
  flips.forEach((range, i) => {
    ...
    for (let i = 0; i <= (end - start) / 2; i++) {
      [newText[start + i], newText[end - i]] = [newText[end - i], newText[start + i]];
      //...
    }
  });
...
app.post('/print/producttypessum', async (req, res) => {
  ...
  clog(pages);
  const customFont = _fs.default.readFileSync(`Tajawal-Light.ttf`);
  doc.registerFont(`Tajawal-Light`, customFont);
  var index1 = 0;
  for (let index = 0; index < pages; index++) {
    //draw header
    doc.image("newlogo.png", 20, 45, {
      width: 200
    }).fillColor("#444444").fontSize(10).text("B2B Corp.", 200, 50, {
      align: "right"
    }).text("kom hamada", 200, 65, {
      align: "right"
    }).text("egypt, behira , 22821", 200, 80, {
      align: "right"
    }).moveDown();

    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Wheights Report", 20, 100);
    generateHr(doc, 125);
    const customerInformationTop = 125;
    doc.fontSize(10).font("Helvetica").text("Report Number:", 20, customerInformationTop + 15).font("Helvetica-Bold").text('WR' + invoicenum, 120, customerInformationTop + 15).font("Helvetica").text("Report Date:", 20, customerInformationTop + 30).font("Helvetica-Bold").text(formatDate(new Date()), 120, customerInformationTop + 30).font("Helvetica").text("Page Number : ", 20, customerInformationTop + 45).font("Helvetica-Bold").text(index + 1, 120, customerInformationTop + 45);
    generateHr(doc, 192);
    ...
    doc.fontSize(10).text(maybeRtlize('م'), 20, invoiceTableTop, {}).text(maybeRtlize('الاسم'), 50, invoiceTableTop, {}).text(maybeRtlize('الكميه'), 490, invoiceTableTop, {
      width: 90,
      align: "right"
    });
    ...
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        doc.fontSize(10).text(index1 + 1, 20, position, {
          features: ['rtla']
        }).text(maybeRtlize(item.name), 50, position, {}).text(item.amount, 490, position, {
          width: 90,
          align: "right"
        });
        generateHr(doc, invoiceTableTop + 10);
  ...
  doc.pipe(_fs.default.createWriteStream('files/' + file));
  res.json({
    'Status': 200,
    file
  });
...
app.post('/print/productssum', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Inventory Report", 20, 100);
    generateHr(doc, 125);
    const customerInformationTop = 125;
    doc.fontSize(10).font("Helvetica").text("Report Number:", 20, customerInformationTop + 15).font("Helvetica-Bold").text('IR' + invoicenum, 120, customerInformationTop + 15).font("Helvetica").text("Report Date:", 20, customerInformationTop + 30).font("Helvetica-Bold").text(formatDate(new Date()), 120, customerInformationTop + 30).font("Helvetica").text("Page Number : ", 20, customerInformationTop + 45).font("Helvetica-Bold").text(index + 1, 120, customerInformationTop + 45);
    generateHr(doc, 192);
    ...
    const invoiceTableTop = 195;
    doc.font("Tajawal-Light");
    doc.fontSize(10).text(maybeRtlize('م'), 20, invoiceTableTop, {}).text(maybeRtlize('الاسم'), 50, invoiceTableTop, {}).text(maybeRtlize('الكميه بالمخزن'), 290, invoiceTableTop, {
      width: 90,
      align: "right"
    }).text(maybeRtlize('الكميه بالثلاجه'), 490, invoiceTableTop, {
      width: 90,
      align: "right"
    });
    generateHr(doc, invoiceTableTop + 10);
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        doc.fontSize(10).text(index1 + 1, 20, position, {
          features: ['rtla']
        }).text(maybeRtlize(item.name), 50, position, {}).text(item.quantity, 290, position, {
          width: 90,
          align: "right"
        }).text(item.famount, 490, position, {
          width: 90,
          align: "right"
        });
        generateHr(doc, invoiceTableTop + 10);
...
app.post('/print/fridgesum', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Fridge Items", 20, 100);
    generateHr(doc, 125);
    const customerInformationTop = 125;
    doc.fontSize(10).font("Helvetica").text("Report Number:", 20, customerInformationTop + 15).font("Helvetica-Bold").text('FI' + invoicenum, 120, customerInformationTop + 15).font("Helvetica").text("Report Date:", 20, customerInformationTop + 30).font("Helvetica-Bold").text(formatDate(new Date()), 120, customerInformationTop + 30).font("Helvetica").text("Page Number : ", 20, customerInformationTop + 45).font("Helvetica-Bold").text(index + 1, 120, customerInformationTop + 45);
    generateHr(doc, 192);
    ...
    doc.fontSize(10).text('id', 20, invoiceTableTop, {
      features: ['rtla']
    }).text('Item Name', 50, invoiceTableTop, {
      features: ['rtla']
    }).text('Quantity', 490, invoiceTableTop, {
      width: 90,
      align: "right"
    });
    generateHr(doc, invoiceTableTop + 10);
    ...
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        doc.fontSize(10).text(index1 + 1, 20, position, {
          features: ['rtla']
        }).text(maybeRtlize(item.to), 50, position, {}).text(item.amount, 490, position, {
          width: 90,
          align: "right"
        });
        generateHr(doc, invoiceTableTop + 10);
...
app.post('/print/clientsum', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Clients Summery", 20, 100);
    generateHr(doc, 125);
    const customerInformationTop = 125;
    doc.fontSize(10).font("Helvetica").text("Report Number:", 20, customerInformationTop + 15).font("Helvetica-Bold").text('CS' + invoicenum, 120, customerInformationTop + 15).font("Helvetica").text("Report Date:", 20, customerInformationTop + 30).font("Helvetica-Bold").text(formatDate(new Date()), 120, customerInformationTop + 30).font("Helvetica").text("Page Number : ", 20, customerInformationTop + 45).font("Helvetica-Bold").text(index + 1, 120, customerInformationTop + 45);
    generateHr(doc, 192);
    ...
    doc.font("Tajawal-Light");
    doc.fontSize(10).text('م', 20, invoiceTableTop, {
      features: ['rtla']
    }).text('الاسم', 50, invoiceTableTop, {
      features: ['rtla']
    }).text('عليه', 270, invoiceTableTop, {
      features: ['rtla']
    }).text('له', 400, invoiceTableTop, {
      features: ['rtla']
    }).text('الرصيد', 490, invoiceTableTop, {
      width: 90,
      align: "right"
    });
    generateHr(doc, invoiceTableTop + 10);
    ...
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        doc.fontSize(10).text(index1 + 1, 20, position, {
          features: ['rtla']
        }).text(maybeRtlize(item.name), 50, position, {}).text(item.payment.toFixed(2), 270, position, {
          width: 90
        }).text(item.expense.toFixed(2), 400, position, {
          width: 90
        }).text((item.payment - item.expense).toFixed(2), 490, position, {
          width: 90,
          align: "right"
        });
        generateHr(doc, invoiceTableTop + 10);
...
app.post('/print/lots', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Imports Summery", 20, 100);
    linelandscape(doc, 125);
    const customerInformationTop = 140;
    doc.fontSize(10).text("Report Number:", 20, customerInformationTop).font("Helvetica-Bold").text('IS' + invoicenum, 120, customerInformationTop).font("Helvetica").text("Report Date:", 20, customerInformationTop + 15).text(formatDate(new Date()), 120, customerInformationTop + 15).text("Page Number : ", 20, customerInformationTop + 30).text(index + 1, 120, customerInformationTop + 30);
    ...
    doc.fontSize(10).text('م', 20, invoiceTableTop, {
      features: ['rtla']
    }).text('المورد', 45, invoiceTableTop, {}).text('الصنف', 185, invoiceTableTop, {}).text('الكميه', 330, invoiceTableTop, {
      features: ['rtla']
    }).text('سعر القطعه', 480, invoiceTableTop, {
      features: ['rtla']
    }).text('السعر الكلي', 610, invoiceTableTop, {
      features: ['rtla']
    }).text('مرتجع؟', 670, invoiceTableTop, {
      features: ['rtla']
    }).text('فاتوره', 710, invoiceTableTop, {
      features: ['rtla']
    }).text('تاريخ الاستلام', 760, invoiceTableTop, {
      features: ['rtla']
    });
    ...
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        doc.font("Tajawal-Light").fontSize(10).text(index1 + 1, 20, position, {
          features: ['rtla']
        }).text(maybeRtlize(item.from), 45, position, {}).text(maybeRtlize(item.to), 185, position, {}).font("Helvetica-Bold").text(item.amount.toFixed(3), 330, position, {
          features: ['rtla']
        }).text(item.price.toFixed(3), 480, position, {
          features: ['rtla']
        }).text((item.amount * item.price).toFixed(3), 610, position, {
          features: ['rtla']
        }).text(item.type == 1 ? "'Y" : "'N", 670, position, {
          features: ['rtla']
        }).text(item.refid, 710, position, {
          features: ['rtla']
        }).text(item.time.split('T')[0], 760, position, {
          features: ['rtla']
        });
    ...
    doc.fontSize(10).text("Thank you for your business.", 160, 560, {
      align: "center",
      width: 500
    });
    if (index < pages - 1) {
...
app.post('/print/exports', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Exports Summery", 20, 100);
    linelandscape(doc, 125);
    const customerInformationTop = 140;
    doc.fontSize(10).text("Report Number:", 20, customerInformationTop).font("Helvetica-Bold").text('ES' + invoicenum, 120, customerInformationTop).font("Helvetica").text("Report Date:", 20, customerInformationTop + 15).text(formatDate(new Date()), 120, customerInformationTop + 15).text("Page Number : ", 20, customerInformationTop + 30).text(index + 1, 120, customerInformationTop + 30);
    ...
    doc.fontSize(10).text('م', 20, invoiceTableTop, {
      features: ['rtla']
    }).text('العميل', 45, invoiceTableTop, {
      features: ['rtla']
    }).text('الصنف', 185, invoiceTableTop, {
      features: ['rtla']
    }).text('الكميه', 490, invoiceTableTop, {
      features: ['rtla']
    }).text('سعر القطعه', 550, invoiceTableTop, {
      features: ['rtla']
    }).text('السعر الكلي', 620, invoiceTableTop, {
      features: ['rtla']
    }).text('فاتوره', 710, invoiceTableTop, {
      features: ['rtla']
    }).text('تاريخ الصرف', 760, invoiceTableTop, {
      features: ['rtla']
    });
    linelandscape(doc, invoiceTableTop + 10);
    doc.font("Tajawal-Light");
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        doc.font("Tajawal-Light").fontSize(10).text(index1 + 1, 20, position, {
          features: ['rtla']
        }).text(maybeRtlize(item.clientname), 45, position).text(maybeRtlize(item.productname), 185, position).font("Helvetica-Bold").text(item.amount, 490, position, {
          features: ['rtla']
        }).text(item.price.toFixed(3), 550, position, {
          features: ['rtla']
        }).text((item.amount * item.price).toFixed(3), 620, position, {
          features: ['rtla']
        }).text(item.refid, 710, position, {
          features: ['rtla']
        }).text(item.time.split('T')[0], 760, position, {
...
app.post('/print/fridgeimports', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Fridge Summery", 20, 100);
    linelandscape(doc, 125);
    const customerInformationTop = 140;
    doc.fontSize(10).text("Report Number:", 20, customerInformationTop).font("Helvetica-Bold").text('FS' + invoicenum, 120, customerInformationTop).font("Helvetica").text("Report Date:", 20, customerInformationTop + 15).text(formatDate(new Date()), 120, customerInformationTop + 15).text("Page Number : ", 20, customerInformationTop + 30).text(index + 1, 120, customerInformationTop + 30);
    ...
    doc.fontSize(10).text('م', 20, invoiceTableTop, {
      features: ['rtla']
    }).text('المورد', 55, invoiceTableTop, {
      features: ['rtla']
    }).text('الصنف', 165, invoiceTableTop, {
      features: ['rtla']
    }).text('الكميه', 320, invoiceTableTop, {
      features: ['rtla']
    }).text('مرتجع', 390, invoiceTableTop, {
      features: ['rtla']
    }).text('اجمالي الكميه', 460, invoiceTableTop, {
      features: ['rtla']
    }).text('سعر القطعه', 530, invoiceTableTop, {
      features: ['rtla']
    }).text('السعر الكلي', 600, invoiceTableTop, {
      features: ['rtla']
    }).text('فاتوره', 720, invoiceTableTop, {
      features: ['rtla']
    }).text('تاريخ الاستلام', 760, invoiceTableTop, {
      features: ['rtla']
    });
    ...
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        const item = invoice.items[index1];
        var position = invoiceTableTop + i * 16;
        doc.font("Tajawal-Light").fontSize(10).text(maybeRtlize(item.from) !== 'اجمالي' ? index1 + 1 : '', 20, position).text(maybeRtlize(item.from), 55, position).text(maybeRtlize(item.to), 165, position).font("Helvetica-Bold").text(item.amount, 320, position, {
          features: ['rtla']
        }).text(item.return, 390, position, {
          features: ['rtla']
        }).text(item.amount - item.return, 460, position, {
          features: ['rtla']
        }).text(item.price, 530, position, {
          features: ['rtla']
        }).text(item.totalprice.toFixed(3), 600, position, {
          features: ['rtla']
        }).text(item.refid, 720, position, {
          features: ['rtla']
        }).text(item.time.split('T')[0], 760, position, {
          features: ['rtla']
        });
...
app.post('/print/vaultsummeryy', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("vault Summery", 20, 100);
    linelandscape(doc, 125);
    const customerInformationTop = 140;
    doc.fontSize(10).text("Report Number:", 20, customerInformationTop).font("Helvetica-Bold").text('VS' + invoicenum, 120, customerInformationTop).font("Helvetica").text("Report Date:", 20, customerInformationTop + 15).text(formatDate(new Date()), 120, customerInformationTop + 15).text("Page Number : ", 20, customerInformationTop + 30).text(index + 1, 120, customerInformationTop + 30);
    ...
    doc.fontSize(10).text('م', 20, invoiceTableTop, {
      features: ['rtla']
    }).text('الخزينه', 35, invoiceTableTop, {
      features: ['rtla']
    }).text('العمليه', 165, invoiceTableTop, {
      features: ['rtla']
    }).text('المعامل', 310, invoiceTableTop, {
      features: ['rtla']
    }).text('وارد', 460, invoiceTableTop, {
      features: ['rtla']
    }).text('منصرف', 550, invoiceTableTop, {
      features: ['rtla']
    }).text('الرصيد', 630, invoiceTableTop, {
      features: ['rtla']
    }).text('فاتوره', 710, invoiceTableTop, {
      features: ['rtla']
    }).text('تاريخ العمليه', 765, invoiceTableTop, {
      features: ['rtla']
    });
    linelandscape(doc, invoiceTableTop + 10);
    doc.font("Tajawal-Light");
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        const item = invoice.items[index1];
        var position = invoiceTableTop + i * 16;
        doc.font("Tajawal-Light").fontSize(10).text(index1 + 1, 20, position, {}).text(fixtext(item.vaultName), 35, position, {
          features: ['ltra']
        }).text(fixtext(item.Category), 165, position, {
          features: ['ltra']
        }).text(fixtext(item.OperatorName), 310, position, {
          features: ['ltra']
        }).font("Helvetica-Bold").text(Number(item.Income) == 0 ? '' : Number(item.Income).toFixed(2).toString().split('.')[1] == '00' ? Number(item.Income) : Number(item.Income).toFixed(2), 460, position, {}).text(Number(item.Outcome) == 0 ? '' : Number(item.Outcome).toFixed(2).toString().split('.')[1] == '00' ? Number(item.Outcome) : Number(item.Outcome).toFixed(2), 550, position, {}).text(Number(item.Value).toFixed(2).toString().split('.')[1] == '00' ? Number(item.Value) : Number(item.Value).toFixed(2), 630, position, {}).text(item.refid, 710, position, {}).text(item.Date, 765, position, {});
        linelandscape(doc, invoiceTableTop + 11);
        linelandscape(doc, position + 11);
        index1++;
        clog(index1);
...
app.post('/print/vaultsummery', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    doc.fontSize(10).font("Helvetica").text("vault Name : ", 20, customerInformationTop).font("Tajawal-Light").text(rows.length > 1 ? rows[0].vaultName.toString() : 'Empty', 120, customerInformationTop, {
      features: ['rtla']
    }).font("Helvetica").text("Report Number:", 20, customerInformationTop + 15).font("Helvetica-Bold").text('VS' + invoicenum, 120, customerInformationTop + 15).font("Helvetica").text("Report Date:", 20, customerInformationTop + 30).text(formatDate(new Date()), 120, customerInformationTop + 30).text("Page Number : ", 20, customerInformationTop + 45).text(index + 1, 120, customerInformationTop + 45);
    ...
    doc.fontSize(10).text('م', 20, invoiceTableTop, {
      features: ['rtla']
    }).text('فاتوره', 55, invoiceTableTop, {
      features: ['rtla']
    }).text('العمليه', 95, invoiceTableTop, {
      features: ['rtla']
    }).text('المعامل', 195, invoiceTableTop, {
      features: ['rtla']
    }).text('وارد', 340, invoiceTableTop, {
      features: ['rtla']
    }).text('منصرف', 400, invoiceTableTop, {
      features: ['rtla']
    }).text('الرصيد', 460, invoiceTableTop, {
      features: ['rtla']
    }).text('تاريخ العمليه', 510, invoiceTableTop, {
      features: ['rtla'],
      align: 'right'
    });
    generateHr(doc, invoiceTableTop + 10);
    ...
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        const item = invoice.items[index1];
        var position = invoiceTableTop + i * 16;
        doc.font("Tajawal-Light").fontSize(10).text(index1 + 1, 20, position, {}).text(item.refid, 55, position, {}).text(maybeRtlize(item.Category), 95, position).text(maybeRtlize(item.OperatorName), 195, position).font("Helvetica-Bold").text(Number(item.Income) == 0 ? '' : Number(item.Income).toFixed(2).toString().split('.')[1] == '00' ? Number(item.Income) : Number(item.Income).toFixed(2), 340, position, {}).text(Number(item.Outcome) == 0 ? '' : Number(item.Outcome).toFixed(2).toString().split('.')[1] == '00' ? Number(item.Outcome) : Number(item.Outcome).toFixed(2), 400, position, {}).text(Number(item.Value).toFixed(2).toString().split('.')[1] == '00' ? Number(item.Value) : Number(item.Value).toFixed(2), 460, position, {}).text(item.Date, 510, position, {
          align: 'right'
        });
    ...
    doc.fontSize(10).text("Thank you for your business.", 50, 800, {
      align: "center",
      width: 500
    });
    if (index < pages - 1) {
...
app.post('/print/clientsummery', async (req, res) => {
  ...
  const pages = Math.ceil(invoice.items.length / rowsperpage);
  clog(pages);
  const customFontt = _fs.default.readFileSync(`Tajawal-Bold.ttf`);
  ...
  for (let index = 0; index < pages; index++) {
    ...
    generateHr(doc, 125);
    const customerInformationTop = 130;
    doc.fontSize(10).font("Helvetica").text("Client Name : ", 20, customerInformationTop).font("Tajawal-Bold").text(rows.length > 1 ? rows[2].clientname : 'Empty', 120, customerInformationTop, {
      features: ['rtla']
    }).font("Helvetica").text("Report Number:", 20, customerInformationTop + 15).font("Helvetica-Bold").text('CS' + invoicenum, 120, customerInformationTop + 15).font("Helvetica").text("Report Date:", 20, customerInformationTop + 30).text(formatDate(new Date()), 120, customerInformationTop + 30).text("Page Number : ", 20, customerInformationTop + 45).text(index + 1, 120, customerInformationTop + 45);
    // .font("Helvetica-Bold")
    // .text(invoice.shipping.name, 300, customerInformationTop)
    // .font("Helvetica")
    // .text(invoice.shipping.address, 300, customerInformationTop + 15)
    ...
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        if (item.id == 'Stop') {
          clog('break');
          doc.fillColor("#000000").fontSize(13).text(iiii == 0 ? "مواد التعبأه المنصرفه " : iiii == 1 ? 'المنتج النهائي الوارد' : iiii == 2 ? 'مواد التعبأه المرتجعه' : iiii == 3 ? 'الرصيد' : 'اجمالي الرصيد', 50, position + 20, {
            align: "right",
            features: ['rtla']
          });
          iiii++;
          ...
          if (iiii == 5 && indexxxx !== 0) {
            doc.fontSize(10).text(indexxxx, 20, invoiceTableTop + (i - 1) * 15 + margin, {
              features: ['rtla']
            }).text(maybeRtlize(item.product.toString()), 50, invoiceTableTop + (i - 1) * 15 + margin).text(maybeRtlize(item.vmoney.toString()), 120, invoiceTableTop + (i - 1) * 15 + margin, {}).text(maybeRtlize(item.cmoney.toString()), 190, invoiceTableTop + (i - 1) * 15 + margin, {}).text(maybeRtlize(item.clientm.toString()), 260, invoiceTableTop + (i - 1) * 15 + margin, {}).text(maybeRtlize(item.vaultm.toString()), 330, invoiceTableTop + (i - 1) * 15 + margin, {}).text(maybeRtlize(item.totalc.toString()), 400, invoiceTableTop + (i - 1) * 15 + margin, {}).text(maybeRtlize(item.totalv.toString()), 470, invoiceTableTop + (i - 1) * 15 + margin, {}).text(maybeRtlize(item.balance.toString()), 490, invoiceTableTop + (i - 1) * 15 + margin, {
              width: 90,
              align: "right",
              features: ['rtla']
            });
            ...
            doc.fontSize(10).text(indexxxx == 0 ? iiii == 2 ? 'م' : 'م' : iiii == 2 ? indexxxx : indexxxx, 20, position, {
              features: ['rtla']
            }).text(indexxxx == 0 ? iiii == 2 ? "الصنف" : "الصنف" : maybeRtlize(item.productname), 50, position).text(indexxxx == 0 ? iiii == 2 ? "الكميه" : '' : iiii == 2 ? item.amount : "", 350, position, {
              width: 90,
              features: ['rtla']
            }).text(indexxxx == 0 ? iiii == 2 ? "مرتجع" : '' : iiii == 2 ? item.return : "", 450, position, {
              width: 90,
              features: ['rtla']
            }).text(indexxxx == 0 ? iiii == 2 ? 'اجمالى الكميه' : 'اجمالى الكميه' : iiii == 2 ? item.amount - item.return : item.amount, 490, position, {
              width: 90,
              align: "right",
              features: ['rtla']
            });
...
app.post('/print/IRH', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Returned Products", 20, 100);
    linelandscape(doc, 125);
    const customerInformationTop = 140;
    doc.fontSize(10).text("Report Number:", 20, customerInformationTop).font("Helvetica-Bold").text('RP' + invoicenum, 120, customerInformationTop).font("Helvetica").text("Report Date:", 20, customerInformationTop + 15).text(formatDate(new Date()), 120, customerInformationTop + 15).text("Page Number : ", 20, customerInformationTop + 30).text(index + 1, 120, customerInformationTop + 30);
...
app.post('/print/clientadvance', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Client Advanced", 20, 100);
    linelandscape(doc, 125);
    const customerInformationTop = 140;
    doc.fontSize(10).text("Report Number:", 20, customerInformationTop).font("Helvetica-Bold").text('CA' + invoicenum, 120, customerInformationTop).font("Helvetica").text("Report Date:", 20, customerInformationTop + 15).text(formatDate(new Date()), 120, customerInformationTop + 15).text("Page Number : ", 20, customerInformationTop + 30).text(index + 1, 120, customerInformationTop + 30);
    ...
    const invoiceTableTop = 195;
    doc.font("Tajawal-Light");
    doc.fontSize(8).text('م', 20, invoiceTableTop, {
      features: ['rtla']
    }).text('العميل', 40, invoiceTableTop, {
      features: ['rtla']
    }).text('الاسم', 165, invoiceTableTop, {
      features: ['rtla']
    }).text('وارد', 280, invoiceTableTop, {
      features: ['rtla']
    }).text('منصرف', 330, invoiceTableTop, {
      features: ['rtla']
    }).text('مرتجع', 380, invoiceTableTop, {
      features: ['rtla']
    }).text('اجمالي الكميه', 430, invoiceTableTop, {
      features: ['rtla']
    }).text('اجمالي المبلغ', 480, invoiceTableTop, {
      features: ['rtla']
    }).text('تحويلات', 530, invoiceTableTop, {
      features: ['rtla']
    }).text('مستحقات', 580, invoiceTableTop, {
      features: ['rtla']
    }).text('العمليه', 630, invoiceTableTop, {
      features: ['rtla']
    }).text('الرصيد', 720, invoiceTableTop, {
      features: ['rtla']
    }).text('التاريخ', 780, invoiceTableTop, {
      features: ['rtla']
    });
    linelandscape(doc, invoiceTableTop + 10);
    doc.font("Tajawal-Light");
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        const item = invoice.items[index1];
        var position = invoiceTableTop + i * 15;
        doc.font("Tajawal-Light").fontSize(8).text(item.id, 20, position).text(item.OperatorName ? maybeRtlize(item.OperatorName) : item.OperatorName, 40, position).text(item.vaultName ? maybeRtlize(item.vaultName) : item.vaultName, 165, position).font("Helvetica-Bold").text(item.Income, 280, position).text(item.Outcome, 330, position, {
          features: ['rtla']
        }).text(item.Return, 380, position, {
          features: ['rtla']
        }).text(isNaN(Number(item.Totalamount)) ? item.Totalamount : item.Totalamount.toFixed(2), 430, position, {
          features: ['rtla']
        }).text(isNaN(Number(item.TotalPrice)) ? item.TotalPrice : item.TotalPrice.toFixed(2), 480, position, {
          features: ['rtla']
        }).text(item.Money, 530, position, {
          features: ['rtla']
        }).text(item.Bonus, 580, position, {
          features: ['rtla']
        }).font("Tajawal-Light").text(item.Notes, 630, position, {
          features: ['rtla']
        }).font("Helvetica-Bold").text(item.Balance, 720, position, {
          features: ['rtla']
        }).text(item.Date, 780, position, {
          features: ['rtla']
        });
        linelandscape(doc, invoiceTableTop + 11);
        index1++;
  ...
  for (let index = 0; index < pages2; index++) {
    ...
    linelandscape(doc, 125);
    const customerInformationTop = 140;
    doc.fontSize(10).text("Report Number:", 20, customerInformationTop).font("Tajawal-Bold").text('CA' + invoicenum, 120, customerInformationTop).font("Helvetica").text("Report Date:", 20, customerInformationTop + 15).text(formatDate(new Date()), 120, customerInformationTop + 15).text("Page Number : ", 20, customerInformationTop + 30).text(index + 1, 120, customerInformationTop + 30);
    ...
    doc.fontSize(8).text('م', 20, invoiceTableTop, {
      features: ['rtla']
    }).text('العمليه', 40, invoiceTableTop, {
      features: ['rtla']
    }).text('منصرف', 165, invoiceTableTop, {
      features: ['rtla']
    }).text('وارد', 280, invoiceTableTop, {
      features: ['rtla']
    }).text('مستحقات له', 380, invoiceTableTop, {
      features: ['rtla']
    }).text('مستحقات عليه', 480, invoiceTableTop, {
      features: ['rtla']
    }).text('اجمالي ما له', 580, invoiceTableTop, {
      features: ['rtla']
    }).text('اجمالي ما عليه', 680, invoiceTableTop, {
      features: ['rtla']
    }).text('الرصيد', 730, invoiceTableTop, {
      width: 90,
      align: "right",
      features: ['rtla']
    });
    ...
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= rows2.length) {
        ...
        var position = invoiceTableTop + i * 15;
        doc.font("Tajawal-Bold").fontSize(8).text(item.id, 20, invoiceTableTop + i * 15 + margin, {
          features: ['rtla']
        }).text(maybeRtlize(item.product), 40, invoiceTableTop + i * 15 + margin).text(!item.id ? maybeRtlize(item.vmoney) : item.vmoney, 165, invoiceTableTop + i * 15 + margin, {}).text(!item.id ? maybeRtlize(item.cmoney) : item.cmoney, 280, invoiceTableTop + i * 15 + margin, {}).text(!item.id ? maybeRtlize(item.clientm) : item.clientm, 380, invoiceTableTop + i * 15 + margin, {}).text(!item.id ? maybeRtlize(item.vaultm) : item.vaultm, 480, invoiceTableTop + i * 15 + margin, {}).text(!item.id ? maybeRtlize(item.totalc) : item.totalc, 580, invoiceTableTop + i * 15 + margin, {}).text(!item.id ? maybeRtlize(item.totalv) : item.totalv, 680, invoiceTableTop + i * 15 + margin, {}).text(!item.id ? maybeRtlize(item.balance) : item.balance ? item.balance.toFixed(2) : '0.00', 730, invoiceTableTop + i * 15 + margin, {
          width: 90,
          align: "right",
          features: ['rtla']
        });
...
app.post('/clientadvance', async (req, res) => {
  ...
  var exportsum = expsum.map(object => ({
    ...object
  }));
...
app.post('/print/workeradvance', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Worker Summery", 20, 100);
    generateHr(doc, 125);
    const customerInformationTop = 130;
    doc.fontSize(10).font("Helvetica").text("Client Name : ", 20, customerInformationTop).font("Tajawal-Bold").text(rows.length > 0 ? rows[0].Name : 'Empty', 120, customerInformationTop, {
      features: ['rtla']
    }).font("Helvetica").text("Report Number:", 20, customerInformationTop + 15).font("Helvetica-Bold").text('WS' + invoicenum, 120, customerInformationTop + 15).font("Helvetica").text("Report Date:", 20, customerInformationTop + 30).font("Helvetica-Bold").text(formatDate(new Date()), 120, customerInformationTop + 30).font("Helvetica").text("Page Number : ", 20, customerInformationTop + 45).font("Helvetica-Bold").text(index + 1, 120, customerInformationTop + 45);
    generateHr(doc, 192);
    ...
    doc.fontSize(10).text('م', 20, invoiceTableTop, {
      features: ['rtla']
    }).text('العمليه', 45, invoiceTableTop, {
      features: ['rtla']
    }).text('عدد العمال', 95, invoiceTableTop, {
      features: ['rtla']
    }).text('اليوميه', 160, invoiceTableTop, {
      features: ['rtla']
    }).text('السهرات', 205, invoiceTableTop, {
      features: ['rtla']
    }).text('الخصم', 260, invoiceTableTop, {
      features: ['rtla']
    }).text('اجمالي المبلغ', 315, invoiceTableTop, {
      features: ['rtla']
    }).text('منصرف', 385, invoiceTableTop, {
      features: ['rtla']
    }).text('الرصيد', 460, invoiceTableTop, {
      features: ['rtla']
    }).text('التاريخ', 540, invoiceTableTop, {
      features: ['rtla'],
      align: 'right'
    });
    generateHr(doc, invoiceTableTop + 10);
    ...
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        const item = invoice.items[index1];
        var position = invoiceTableTop + i * 16;
        doc.font("Tajawal-Bold").fontSize(10).text(index1 + 1, 20, position).text(maybeRtlize(item.Type), 45, position).font("Helvetica-Bold").text(item.Count, 95, position).text(item.Amount, 160, position, {
          features: ['rtla']
        }).text(item.Nights, 205, position, {
          features: ['rtla']
        }).text(item.Return, 260, position, {
          features: ['rtla']
        }).text(item.dadd, 315, position, {
          features: ['rtla']
        }).text(item.dsub, 385, position, {
          features: ['rtla']
        }).text(item.Balance, 460, position, {
          features: ['rtla']
        }).text(item.Date, 520, position, {
          features: ['rtla'],
          align: 'right'
        });
        generateHr(doc, invoiceTableTop + 11);
...
app.post('/print/vaultclienttransactions', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Client Transactions", 20, 100);
    generateHr(doc, 125);
    const customerInformationTop = 125;
    doc.fontSize(10).font("Helvetica").font("Tajawal-Bold").text(rows.length > 0 ? rows[0].Name : 'Empty', 120, customerInformationTop, {
      features: ['rtla']
    }).font("Helvetica").text("Report Number:", 20, customerInformationTop + 15).font("Helvetica-Bold").text('CT' + invoicenum, 120, customerInformationTop + 15).font("Helvetica").text("Report Date:", 20, customerInformationTop + 30).font("Helvetica-Bold").text(formatDate(new Date()), 120, customerInformationTop + 30).font("Helvetica").text("Page Number : ", 20, customerInformationTop + 45).font("Helvetica-Bold").text(index + 1, 120, customerInformationTop + 45);
    generateHr(doc, 192);
    ...
    doc.fontSize(10).text('م', 20, invoiceTableTop, {
      features: ['rtla']
    }).text('فاتوره', 45, invoiceTableTop, {
      features: ['rtla']
    }).text('الاسم', 105, invoiceTableTop, {
      features: ['rtla']
    }).text('وارد', 315, invoiceTableTop, {
      features: ['rtla']
    }).text('منصرف', 430, invoiceTableTop, {
      features: ['rtla']
    }).text('التاريخ', 540, invoiceTableTop, {
      features: ['rtla'],
      align: 'right'
    ...
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        const item = invoice.items[index1];
        var position = invoiceTableTop + i * 16;
        doc.font("Tajawal-Bold").fontSize(10).text(index1 + 1, 20, position).text(maybeRtlize(item.toname), 100, position).font("Helvetica-Bold").text(item.refid, 45, position, {
          features: ['rtla']
        }).text(item.add, 315, position, {
          features: ['rtla']
        }).text(item.sub, 430, position, {
          features: ['rtla']
        }).text(item.time.toString().split('T')[0], 520, position, {
          features: ['rtla'],
          align: 'right'
        });
        generateHr(doc, invoiceTableTop + 11);
...
app.post('/print/moneyownertransactions', async (req, res) => {
  ...
  for (let index = 0; index < pages; index++) {
    ...
    // draw detaild
    doc.fillColor("#444444").fontSize(20).text("Investor Transactions", 20, 100);
    generateHr(doc, 125);
    const customerInformationTop = 125;
    doc.fontSize(10).font("Helvetica").font("Tajawal-Bold").text(rows.length > 0 ? rows[0].Name : 'Empty', 120, customerInformationTop, {
      features: ['rtla']
    }).font("Helvetica").text("Report Number:", 20, customerInformationTop + 15).font("Helvetica-Bold").text('IT' + invoicenum, 120, customerInformationTop + 15).font("Helvetica").text("Report Date:", 20, customerInformationTop + 30).font("Helvetica-Bold").text(formatDate(new Date()), 120, customerInformationTop + 30).font("Helvetica").text("Page Number : ", 20, customerInformationTop + 45).font("Helvetica-Bold").text(index + 1, 120, customerInformationTop + 45);
    generateHr(doc, 192);
    ...
    doc.fontSize(10).text('م', 20, invoiceTableTop, {
      ...
      features: ['rtla']
    }).text('الشريك', 105, invoiceTableTop, {
      features: ['rtla']
    }).text('الخزينه', 205, invoiceTableTop, {
      features: ['rtla']
    }).text('وارد', 345, invoiceTableTop, {
      features: ['rtla']
    }).text('منصرف', 430, invoiceTableTop, {
      features: ['rtla']
    }).text('التاريخ', 540, invoiceTableTop, {
    ...
    for (i = 1; i <= rowsperpage; i++) {
      if (index1 >= invoice.items.length) {
        ...
        const item = invoice.items[index1];
        var position = invoiceTableTop + i * 16;
        doc.font("Tajawal-Bold").fontSize(10).text(item.id, 20, position).text(maybeRtlize(item.toname), 100, position).text(maybeRtlize(item.fromname), 200, position).text(maybeRtlize(item.refid), 45, position, {
          features: ['rtla']
        }).font("Helvetica-Bold").text(item.add, 345, position, {
          features: ['rtla']
        }).text(item.sub, 430, position, {
          features: ['rtla']
        }).text(item.time.toString().split('T')[0], 520, position, {
...
function generateHeader(doc) {
  doc.image("logo.png", 50, 45, {
    width: 50
  }).fillColor("#444444").fontSize(20).text("B2B Corp.", 110, 57).fontSize(10).text("B2B Corp.", 200, 50, {
    align: "right"
  }).text("kom hamada", 200, 65, {
    align: "right"
  }).text("egypt, behira , 22821", 200, 80, {
...
function generateCustomerInformation(doc, invoice) {
  ...
  doc.fontSize(10).text("Invoice Number:", 50, customerInformationTop).font("Helvetica-Bold").text(invoice.invoice_nr, 150, customerInformationTop).font("Helvetica").text("Invoice Date:", 50, customerInformationTop + 15).text(formatDate(new Date()), 150, customerInformationTop + 15).text("Balance Due:", 50, customerInformationTop + 30).text(formatCurrency(invoice.subtotal - invoice.paid), 150, customerInformationTop + 30).font("Helvetica-Bold").text(invoice.shipping.name, 300, customerInformationTop).font("Helvetica").text(invoice.shipping.address, 300, customerInformationTop + 15).text(invoice.shipping.city + ", " + invoice.shipping.state + ", " + invoice.shipping.country, 300, customerInformationTop + 30).moveDown();
  generateHr(doc, 252);
}
...
function generateTableRow(doc, y, item, description, unitCost, quantity, lineTotal, id) {
  doc.fontSize(10).text(id, 50, y, {
    features: ['rtla']
  }).text(item, 80, y, {
    features: ['rtla']
  }).text(description, 190, y).text(unitCost, 280, y, {
    width: 90,
    align: "right"
  }).text(quantity, 370, y, {
    width: 90,
...
function linelandscape(doc, y) {
  doc.strokeColor("#aaaaaa").lineWidth(1).moveTo(20, y).lineTo(830, y).stroke();
}
...
function generateAccessToken() {
  const randomBytess = (0, _crypto.randomBytes)(32).toString('hex');
  const timestamp = Date.now();
  const accessToken = (0, _crypto.createHmac)('sha256', randomBytess).update(timestamp.toString()).digest('hex');
  return accessToken;
}
app.post('/login', async (req, res) => {
  ...
  const hash = (0, _crypto.createHash)('md5').update(password).digest('hex');
...
app.post('/addproduct', async (req, res) => {
  ...
  if (selectmode == 'unit') {
    mesure = 0;
  } else if (selectmode == 'kg') {
    mesure = 1;
  } else if (selectmode == 'M') {
    mesure = 2;
  } else if (selectmode == 'm2') {
    mesure = 3;
  }
  clog(mesure);
...
app.post('/productimportrefid', async (req, res) => {
  ...
  clog({
    refid,
    count: newexport.length,
    editing: newexport.length > 0,
    used: false
  });
  res.status(200).json({
    "status": 200,
...
app.post('/clientsummery', async (req, res) => {
  ...
  const exportsumedit = expsum.map(object => ({
    ...object
  }));
  const exportsum = expsum.map(object => ({
    ...object
  }));
  impsum = impsum.map(({
    toid: productid,
...
app.post('/productimportreturnrefid', async (req, res) => {
  ...
  clog('product import return refid search', {
    refid,
    count: newexport.length,
    editing: newexport.length > 0,
    used: false
...
const dd = async () => {
  ...
  await prisma.moneyowner.updateMany({
    data: {
      payed: 0,
      payment: 0,
      value: 0,
...
// dd()
// calc_fridge()
app.listen(1024, () => clog(`b2b app listening on port ${1024}!`));
