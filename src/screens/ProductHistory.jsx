...
function ProductHistory() {
    ...
    const searchclient = () => {
        setloadrn(true);
        axios.post('http://192.168.1.20:1024/searchproducthistoryexactbyclient', { searchtext: firstvaultname }).then((resp) => {
            if (resp.data.status == 200) {
                setrows(resp.data.foundproduts)
                var sum = 0
                var asum = 0
    ...
    const actions = () => (
        <div style={{ width: '100%', alignItems: 'flex-end', display: 'flex', justifyContent: 'end', flexDirection: 'row' }}>
            ...
            <Button style={{ margin: 10 }} disabled={refreshloading} variant='contained' color='warning' onClick={() => {
                ...
                axios.get('http://192.168.1.20:1024/Producthistory').then((resp) => { setrows(resp.data.products); setrefreshloading(false) })
