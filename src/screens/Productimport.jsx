...
function Productimport() {
    ...
    const createnewexport = () => {
        ...
        setsubmit(true)
        console.log(rows)
        axios.post('http://192.168.1.20:1024/addProductimport', { rows, refid, client: firstvaultname, time: newexpenses }).then((resp) => {
            if (resp.data.status == 200) {
                console.log(resp.data)
                setsecondvaultname('')
                setnewamount(0)
    ...
    const editsubmit = () => {
        ...
        setsubmit(true)
        console.log(rows)
        axios.post('http://192.168.1.20:1024/editProductimport', { rows, refid, client: firstvaultname, time: newexpenses }).then((resp) => {
            if (resp.data.status == 200) {
                console.log(resp.data)
                setsecondvaultname('')
                setnewamount(0)
    ...
    const searchrefid = (refid) => {
        setloadrn(true);
        axios.post('http://192.168.1.20:1024/productimportrefid', { refid: refid }).then((resp) => {
            if (resp.data.status == 200) {
                setloadrn(false);
                console.log(resp.data)
            ...
            <div style={{ width: '100%' }}>
                <div style={{ width: '100%', alignItems: 'baseline', display: 'flex', justifyContent: 'start', flexDirection: 'row' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline' }}>
                        ...
                        <Button disabled={submit} style={{ marginRight: 20 }} variant='contained' onClick={() => { searchrefid(refid) }} color='warning'>بحث</Button>
                        <Button disabled={submit} style={{ marginRight: 20 }} variant='contained' onClick={() => {
                            setsubmit(true)
                            axios.post('http://192.168.1.20:1024/deleteProductimport', { refid: refid }).then((resp) => {
                                if (resp.data.status == 200) {
                                    setrows([])
                                    setsecondvaultname('')
                                    setnewamount(0)
                        ...
                        <Checkbox style={{ marginLeft: 20 }} checked={eden} label="تعديل" />
                        <Checkbox style={{ marginLeft: 20 }} checked={used} label="مستخدمة بالفعل" />
                    </div>
            ...
            <Dialog open={editrn} onClose={() => { seteditrn(false) }}>
                ...
                <DialogActions>
                    ...
                    <Button variant='contained' onClick={() => {
                        ...
                        arr[selindex].totalprice = editprice * newcamount
                        setrows(arr)
                        seteditrn(false)
                    }
                    }>حفظ</Button>
