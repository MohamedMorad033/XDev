...
function Productimport() {
    ...
    const createnewexport = () => {
        ...
        setsubmit(true)
        console.log(rows)
        axios.post('http://localhost:1024/addProductimport', { rows, refid, client: firstvaultname, time: newexpenses }).then((resp) => {
            if (resp.data.status == 200) {
                console.log(resp.data)
                setsecondvaultname('')
                setnewamount(0)
    ...
    const editsubmit = () => {
        ...
        setsubmit(true)
        console.log(rows)
        axios.post('http://localhost:1024/editProductimport', { rows, refid, client: firstvaultname, time: newexpenses }).then((resp) => {
            if (resp.data.status == 200) {
                console.log(resp.data)
                setsecondvaultname('')
                setnewamount(0)
    ...
    const searchrefid = (refid) => {
        setloadrn(true);
        axios.post('http://localhost:1024/productimportrefid', { refid: refid }).then((resp) => {
            if (resp.data.status == 200) {
                setloadrn(false);
                console.log(resp.data)
                ...
                <div style={{ width: '100%', alignItems: 'baseline', display: 'flex', justifyContent: 'start', flexDirection: 'row' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline' }}>
                        ...
                        <Button disabled={submit} style={{ marginRight: 20 }} variant='contained' onClick={() => {
                            setsubmit(true)
                            axios.post('http://localhost:1024/deleteProductimport', { refid: refid }).then((resp) => {
                                if (resp.data.status == 200) {
                                    setrows([])
                                    setsecondvaultname('')
                                    setnewamount(0)
