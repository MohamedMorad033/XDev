...
function FinalExportHistory() {
    ...
    const editlot = () => {
        ...
        console.log(newdata)
        axios.post('http://localhost:1024/editFinalexportlot', { lotid: newdata.id, newprice: newpayments, newamount: newexpenses }).then((resp) => {
            if (resp.data.status == 200) {
                console.log(resp.data)
    ...
    const searchprod = () => {
        setloadrn(true);
        axios.post('http://localhost:1024/searchautoexportsexact', { searchtext: secondvaultname }).then((resp) => {
            if (resp.data.status == 200) {
    ...
    const searchrefid = () => {
        setloadrn(true);
        axios.post('http://localhost:1024/searchautoexportsbyrefid', { refid }).then((resp) => {
            if (resp.data.status == 200) {
                setrows(resp.data.foundproduts)
                var sum = 0
                var asum = 0
    ...
    const searchclient = () => {
        setloadrn(true);
        axios.post('http://localhost:1024/searchautoexportsexactbyclient', { searchtext: firstvaultname }).then((resp) => {
            if (resp.data.status == 200) {
                setrows(resp.data.foundproduts)
                var sum = 0
                var asum = 0
    ...
    const generalsearch = (cln, prd, rfd, dat, fdt) => {
        setsearchload(true);
        axios.post('http://localhost:1024/searchfinalexportgeneral', { product: prd, client: cln, refid: rfd, date: dat, fdate: fdt }).then((resp) => {
            if (resp.data.status == 200) {
...
export default FinalExportHistory