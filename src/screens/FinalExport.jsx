...
function FinalExport() {
    ...
    const editsubmit = () => {
        ...
        setsubmit(true)
        console.log(rows)
        axios.post('http://localhost:1024/finalexportproducts', { rows, refid, clientname: firstvaultname, time: newexpenses }).then((resp) => {
            if (resp.data.status == 200) {
                console.log(resp.data)
                setsecondvaultname('')
                setnewamount(0)
    ...
    const searchprod = (data) => {
        setloadrn(true);
        axios.post('http://localhost:1024/searchfridgeamountexact', { searchtext: data }).then((resp) => {
            if (resp.data.status == 200) {
    ...
    const searchrefid = (refid) => {
        setloadrn(true);
        axios.post('http://localhost:1024/finalexportrefid', { refid: refid }).then((resp) => {
            if (resp.data.status == 200) {
                setloadrn(false);
                ...
                <div style={{ width: '100%', alignItems: 'baseline', display: 'flex', justifyContent: 'start', flexDirection: 'row' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline' }}>
                        ...
                        <Button disabled={submit} style={{ marginRight: 20 }} variant='contained' onClick={() => {
                            setsubmit(true)
                            axios.post('http://localhost:1024/finalexportdelete', { refid: refid }).then((resp) => {
                                if (resp.data.status == 200) {
                                    setrows([])
                                    setsecondvaultname('')
                                    setnewamount(0)
