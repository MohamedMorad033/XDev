...
function Productexportreturn() {
    ...
    const columns = [
        ...
        {
            name: 'السعر الكلي',
            selector: row => row.return * row.price,
            sortable: true,
        }];
    ...
    const editsubmit = () => {
        ...
        // setsubmit(true)
        console.log(rows)
        axios.post('http://192.168.1.20:1024/editproductexportreturn', { rows, refid, client: firstvaultname, time: newexpenses }).then((resp) => {
            if (resp.data.status == 200) {
                console.log(resp.data)
                setsecondvaultname('')
                setnewamount(0)
    ...
    const searchprod = (data) => {
        setloadrn(true);
        axios.post('http://192.168.1.20:1024/searchproducthistorybyclientexact', { searchtext: data, clientname: firstvaultname }).then((resp) => {
            if (resp.data.status == 200) {
    ...
    const searchrefid = () => {
        setloadrn(true);
        axios.post('http://192.168.1.20:1024/productexportreturnrefid', { refid: refid }).then((resp) => {
            if (resp.data.status == 200) {
                setloadrn(false);
    ...
    const addtolist = () => {
        ...
        console.log({ lotid, name, amount, price, client: firstvaultname, newamount })
        ...
        <div style={{ width: '100%', alignItems: 'baseline', display: 'flex', justifyContent: 'end', flexDirection: 'row' }}>
            <div style={{ display: 'flex', alignItems: 'baseline' }}>
                ...
                <Autocomplete
                    ...
                    size='small'
                    renderInput={(params) => <TextField {...params} label="الصrrrrrrنف" />}
                />
                <Autocomplete
                    id="free-solo-demo"
                    label='d'
                    ...
                    options={lots.map((option) => option.id + '_____' + option.productname + '_____' + option.remaining + '_____' + option.price)}
                    size='small'
            ...
            <Dialog open={editrn} onClose={() => { seteditrn(false) }}>
                ...
                <DialogActions>
                    ...
                    <Button variant='contained' onClick={() => {
                        var arr = rows
                        arr[selindex].return = newcamount
...
export default Productexportreturn