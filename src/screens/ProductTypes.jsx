...
function ProductTypes() {
    ...
    const editsubmit = () => {
        setloadrn(true)
        axios.post('http://localhost:1024/editproducttypes', { name, amount, selid }).then((resp) => {
            if (resp.data.status == 200) {
                console.log(resp.data)
                setrows([])
                setloadrn(false)
                seteditrn(false)
                axios.get('http://localhost:1024/producttypes').then((resp) => { setrows(resp.data.products) })
            } else {
                setloadrn(false)
                alert('failed')
            }
    ...
    const createnew = () => {
        ...
        setloadrn(true)
        axios.post('http://localhost:1024/addproducttype', { name: newname, amount: newamount }).then((resp) => {
            if (resp.data.status == 200) {
                console.log(resp.data)
                setrows([])
                setloadrn(false)
    ...
    const search = (text) => {
        setsearchload(true)
        axios.post('http://localhost:1024/searchproducttypes', { searchtext: text }).then((resp) => {
            if (resp.data.status == 200) {
                setsearchload(false)
                console.log(resp.data)
                setrows([])
    ...
    const contextActions = () => (
        <>
            ...
            <Button color='success' style={{ marginRight: 20 }} variant='contained' onClick={() => {
                axios.post('http://localhost:1024/print/producttypessum', {}).then((resp) => {
                    setTimeout(() => {
    ...
    const actions = () => (
        <div style={{ width: '100%', alignItems: 'baseline', display: 'flex', justifyContent: 'start', flexDirection: 'row' }}>
            <TextField
                ...
                variant="outlined"
            />
            <Button disabled={newloadrn} color='success' variant='contained' onClick={() => { createnew() }}>تأكيد</Button>
            <Button style={{ marginLeft: 20 }} disabled={refreshloading} variant='contained' color='warning' onClick={() => {
                setrefreshloading(true);
                axios.get('http://localhost:1024/producttypes').then((resp) => { setrows(resp.data.products); setrefreshloading(false) })
            ...
            <div style={{ width: '100%', display: 'flex', marginTop: 10, alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', marginTop: 10, alignItems: 'baseline', width: '100%', justifyContent: 'end' }}>
                    ...
                    <Button style={{ margin: 20 }} disabled={searchload} variant='contained' color='error' onClick={() => {
                        setsearchtext(''); axios.get('http://localhost:1024/producttypes').then((resp) => { setrows(resp.data.products) })
                    }}>Clear</Button>
                </div>
            </div>
            <div style={{ width: '100%' }}>
