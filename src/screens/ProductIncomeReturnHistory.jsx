...
function ProductIncomeReturnHistory() {
    ...
    const actions = [
        ...
        { icon: <SaveIcon onClick={(e) => { exportToCsv(e) }} />, name: 'Save' },
        {
            icon: <PrintIcon onClick={(e) => {
                axios.post('http://localhost:1024/print/IRH', { rows: rows, rows2: rows }).then((resp) => {
                    setTimeout(() => {
    ...
    const editlot = () => {
        ...
        console.log(newdata)
        axios.post('http://localhost:1024/editimportreturnlot', { lotid: newdata.id, newprice: newpayments, newamount: newexpenses }).then((resp) => {
            if (resp.data.status == 200) {
                console.log(resp.data)
    ...
    const generalsearch = (cln, prd, rfd, dat, fdt) => {
        setsearchload(true);
        axios.post('http://localhost:1024/searchproductoutcomereturnhistorygeneral', { product: prd, client: cln, refid: rfd, date: dat, fdate: fdt }).then((resp) => {
            if (resp.data.status == 200) {
        ...
        <div style={{ width: '100%', flexDirection: 'row', display: 'flex', alignItems: 'flex-start' }}>
            ...
            <div style={{ width: filter ? '100%' : '80%', minHeight: window.innerHeight - 46, display: 'flex', flexDirection: 'column' }} onClick={() => { setfilter(true) }}>
                ...
                <Dialog open={editrn} onClose={() => { seteditrn(false) }}>
                    <DialogTitle>تعديل فاتوره مرتجع مشتريات</DialogTitle>
                    <DialogContent>
                        <TextField
                            autoFocus
                            margin="dense"
...
export default ProductIncomeReturnHistory