...
function Productincomereturn() {
    ...
    const columns = [
        ...
        {
            name: 'السعر الكلي',
            selector: row => row.amount * row.price,
            sortable: true,
        }];
    ...
    const editsubmit = () => {
        ...
        setsubmit(true)
        console.log(rows)
        axios.post('http://localhost:1024/editproductincomereturn', { rows, refid, clientname: firstvaultname, time: newexpenses }).then((resp) => {
            if (resp.data.status == 200) {
                console.log(resp.data)
                setsecondvaultname('')
                setnewamount(0)
    ...
    const searchrefid = (refid) => {
        setloadrn(true);
        axios.post('http://localhost:1024/productimportreturnrefid', { refid: refid }).then((resp) => {
            if (resp.data.status == 200) {
                setloadrn(false);
    ...
    const addtolist = () => {
        ...
        setrows(oldArray => [...oldArray, {
            ...
            "amount": newamount,
            "price": average,
            "totalprice": average * totalamount,
            "return": 0,
            "returnid": 0
        }])
        setsecondvaultname('')
                ...
                <div style={{ width: '100%', alignItems: 'baseline', display: 'flex', justifyContent: 'start', flexDirection: 'row' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline' }}>
                        ...
                        <Button disabled={submit} style={{ marginRight: 20 }} variant='contained' onClick={() => {
                            setsubmit(true)
                            axios.post('http://localhost:1024/productincomereturndelete', { refid: refid }).then((resp) => {
                                if (resp.data.status == 200) {
                                    setrows([])
                                    setsecondvaultname('')
                                    setnewamount(0)
...
export default Productincomereturn